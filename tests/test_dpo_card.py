...
def _write_consolidated_card(tmp_path: Path) -> Path:
    ...
    readme.write_text(
        build_dataset_card("dpo", total=2, signals=FAMILIES),
        encoding="utf-8",
    )
    return readme
...
def test_consolidated_dpo_default_and_family_configs_reference_same_files(tmp_path):
    data_dir = tmp_path / "data"
    data_dir.mkdir()
    for family in FAMILIES:
        (data_dir / f"{family}.jsonl").write_text("{}\n", encoding="utf-8")
    configs = load_dpo_dataset_card_configs(_write_consolidated_card(tmp_path))
