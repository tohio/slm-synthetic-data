...
def _write_run_manifest(path, dataset, row_count) -> None:
    path.write_text(
        json.dumps(
            {
                "datasets": [{"signal": "cloud", "dataset_path": str(dataset), "row_count": row_count}],
                "total_rows": row_count,
                "metadata": {"accepted_rows": row_count},
            }
        ),
        encoding="utf-8",
    )
...
def test_adjudication_keeps_valid_control_and_quarantines_other_cluster_members(tmp_path):
    ...
    _write_run_manifest(run_manifest, dataset, 29)
    ...
    assert manifest["metadata"]["generated_accepted_rows"] == 29
    assert manifest["metadata"]["curation_rejected_rows"] == 28
