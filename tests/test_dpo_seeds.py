...
def test_build_seed_rows_dispatches_ai_concept_family():
    ...
    assert rows[0]["id"] == "dpo_ai_concept_explanation_000007"
...
def test_build_seed_rows_dispatches_code_generation_family():
    ...
    assert rows[0]["id"] == "dpo_code_generation_function_000007"
...
def test_build_seed_rows_dispatches_function_completion_family():
    ...
    assert rows[0]["id"] == "dpo_function_completion_body_only_000007"
...
def test_build_seed_rows_dispatches_code_explanation_family():
    ...
    assert rows[0]["id"] == "dpo_code_explanation_no_code_000007"
...
def test_build_ai_concept_explanation_rows_creates_valid_dpo_rows():
    rows = build_ai_concept_explanation_rows(count=2)

    assert [row["id"] for row in rows] == [
        "dpo_ai_concept_explanation_000001",
        "dpo_ai_concept_explanation_000002",
    ]
    assert rows[0]["prompt"][0]["content"] == "In machine learning, what is an attention mechanism?"
    assert rows[0]["chosen"][0]["content"] == (
        "It helps a model focus on the most relevant parts of the input when producing an output."
    )
    assert rows[0]["rejected"][0]["content"] == (
        "It is the electrical power unit inside a computer that converts voltage for AI hardware."
    )
...
def test_build_ai_concept_explanation_rows_skips_holdout_keys():
    ...
    assert rows[0]["prompt"][0]["content"] != "In machine learning, what is an attention mechanism?"
    assert rows[0]["id"] == "dpo_ai_concept_explanation_000001"
...
def test_build_ai_concept_explanation_rows_allows_sibling_not_eval_holdout():
    ...
    assert "transformer" not in rows[0]["prompt"][0]["content"]
    assert "neural network" not in rows[0]["prompt"][0]["content"]
    assert rows[0]["chosen"][0]["content"].startswith("It helps a model focus")
    assert rows[0]["chosen"] != rows[0]["rejected"]
...
def test_build_direct_subtraction_rows_creates_valid_dpo_rows():
    rows = build_direct_subtraction_rows(count=2)

    assert [row["id"] for row in rows] == [
        "dpo_direct_subtraction_000001",
        "dpo_direct_subtraction_000002",
    ]
    assert rows[0]["prompt"][0]["content"] == "What is 13 - 2?"
    assert rows[0]["chosen"][0]["content"] == "11"
    assert rows[0]["rejected"][0]["content"] == "12"
...
def test_build_direct_subtraction_rows_skips_holdout_keys():
    ...
    assert rows[0]["prompt"][0]["content"] != "What is 13 - 2?"
    assert rows[0]["id"] == "dpo_direct_subtraction_000001"
...
def test_build_code_generation_function_rows_creates_valid_dpo_rows():
    rows = build_code_generation_function_rows(count=2)

    assert [row["id"] for row in rows] == [
        "dpo_code_generation_function_000001",
        "dpo_code_generation_function_000002",
    ]
    assert rows[0]["prompt"][0]["content"] == (
        "Write a Python function named multiply that multiplies two numbers. Return only code."
    )
    assert rows[0]["chosen"][0]["content"] == "def multiply(a, b):\n    return a * b"
    assert rows[0]["rejected"][0]["content"] == "Here is the function:\n\ndef multiply(a, b):\n    return a * b"
    assert rows[0]["metadata"]["category"] == "code_generation"
    assert rows[0]["metadata"]["failure_mode"] == "code_includes_explanation"
...
def test_build_code_generation_function_rows_skips_holdout_keys():
    ...
    assert rows[0]["id"] == "dpo_code_generation_function_000001"
...
def test_build_code_generation_function_rows_allows_sibling_not_eval_holdout():
    ...
    assert "square" not in rows[0]["prompt"][0]["content"]
    assert rows[0]["chosen"][0]["content"].startswith("def multiply")
    assert rows[0]["chosen"] != rows[0]["rejected"]


def test_build_function_completion_body_only_rows_creates_valid_dpo_rows():
    rows = build_function_completion_body_only_rows(count=2)

    assert [row["id"] for row in rows] == [
        "dpo_function_completion_body_only_000001",
        "dpo_function_completion_body_only_000002",
    ]
    assert "def double(x):" in rows[0]["prompt"][0]["content"]
    assert rows[0]["chosen"][0]["content"] == "return x * 2"
    assert rows[0]["rejected"][0]["content"] == "This function should return the requested value.\n\nreturn x * 2"
    assert rows[0]["metadata"]["category"] == "code_generation"
    assert rows[0]["metadata"]["failure_mode"] == "code_includes_explanation"
...
def test_build_function_completion_body_only_rows_skips_holdout_keys():
    ...
    assert "def double(x):" not in rows[0]["prompt"][0]["content"]
    assert rows[0]["id"] == "dpo_function_completion_body_only_000001"
...
def test_build_function_completion_body_only_rows_allows_sibling_not_eval_holdout():
    ...
    assert "first_item" not in rows[0]["prompt"][0]["content"]
    assert "has_close_elements" not in rows[0]["prompt"][0]["content"]
    assert rows[0]["chosen"][0]["content"] == "return x * 2"
    assert rows[0]["chosen"] != rows[0]["rejected"]


def test_build_code_explanation_no_code_rows_creates_valid_dpo_rows():
    rows = build_code_explanation_no_code_rows(count=2)

    assert [row["id"] for row in rows] == [
        "dpo_code_explanation_no_code_000001",
        "dpo_code_explanation_no_code_000002",
    ]
    assert "def double" in rows[0]["prompt"][0]["content"]
    assert rows[0]["chosen"][0]["content"] == "It returns the input value multiplied by two."
    assert "def double" not in rows[0]["chosen"][0]["content"]
    assert rows[0]["rejected"][0]["content"] == (
...
def test_build_code_explanation_no_code_rows_skips_holdout_keys():
    ...
    assert "def double" not in rows[0]["prompt"][0]["content"]
    assert rows[0]["id"] == "dpo_code_explanation_no_code_000001"
