...
def test_build_seed_rows_dispatches_direct_division_family():
    ...
    assert rows[0]["id"] == "sft_direct_division_000007"
...
def test_build_direct_division_rows_creates_valid_sft_rows():
    rows = build_direct_division_rows(count=2)

    assert [row["id"] for row in rows] == [
        "sft_direct_division_000001",
        "sft_direct_division_000002",
    ]
    assert rows[0]["messages"][0]["content"] == "What is 6 / 2?"
    assert rows[0]["messages"][1]["content"] == "3"
    assert rows[0]["metadata"]["category"] == "direct_arithmetic"
...
def test_build_direct_division_rows_skips_holdout_keys():
    ...
    assert rows[0]["messages"][0]["content"] != "What is 6 / 2?"
    assert rows[0]["id"] == "sft_direct_division_000001"
...
def test_build_direct_division_rows_allows_sibling_not_eval_holdout():
    ...
    assert "20 / 5" not in rows[0]["messages"][0]["content"]
    assert rows[0]["messages"][1]["content"] == "3"
