...
def build_direct_subtraction_rows(
    *,
    ...
    """Build direct subtraction SFT rows without using held-out items."""
    if not isinstance(count, int) or count < 1:
        raise ValueError("count must be a positive integer")
    if not isinstance(start_index, int) or start_index < 1:
        raise ValueError("start_index must be a positive integer")
    ...
    for candidate in _direct_subtraction_candidates():
        ...
        row = {
            "id": f"sft_direct_subtraction_{row_number:06d}",
            "messages": [
                {"role": "user", "content": candidate["prompt"]},
                {"role": "assistant", "content": str(candidate["answer"])},
            ],
    ...
    raise ValueError(f"could not build {count} direct-subtraction SFT rows")
...
def build_direct_division_rows(
    *,
    ...
    """Build direct integer-division SFT rows without using held-out items."""
    if not isinstance(count, int) or count < 1:
        raise ValueError("count must be a positive integer")
    if not isinstance(start_index, int) or start_index < 1:
        raise ValueError("start_index must be a positive integer")
    ...
    for candidate in _direct_division_candidates():
        ...
        row = {
            "id": f"sft_direct_division_{row_number:06d}",
            "messages": [
                {"role": "user", "content": candidate["prompt"]},
                {"role": "assistant", "content": str(candidate["answer"])},
            ],
    ...
    raise ValueError(f"could not build {count} direct-division SFT rows")
