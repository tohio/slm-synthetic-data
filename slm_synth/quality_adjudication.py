...
def render_quality_adjudication_prompt(
    *, dataset_type: str, specs: Iterable[Mapping[str, Any]], rows: Iterable[Mapping[str, Any]]
    ...
    return (
        ...
        "appropriate invented details are allowed unless an explicit requirement prohibits them; judge consistency with the "
        "brief rather than demanding that every fictional detail appear in variables. Treat meaning-preserving edits as "
        "preserving uncertainty and limitations even when wording changes. Enforce explicit word counts, item counts, headings, "
