...
def build_coverage_report(run_manifest_path: str | Path) -> dict[str, Any]:
    ...
    return {
        ...
        "generated_accepted_rows": metadata.get(
            "generated_accepted_rows",
            metadata.get("accepted_rows", total_rows),
        ),
        "accepted_rows": metadata.get("accepted_rows", total_rows),
        "rejected_rows": metadata.get("rejected_rows"),
        "curation_rejected_rows": metadata.get("curation_rejected_rows", 0),
        "row_count": total_rows,
        "signals": {signal: signal_counts[signal] for signal in sorted(signal_counts)},
        "candidate_rows_per_signal": _sorted_mapping(metadata.get("candidate_rows_per_signal")),
