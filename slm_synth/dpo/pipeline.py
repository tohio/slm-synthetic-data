#!/usr/bin/env python3
"""Production generic DPO pipeline migrated from the finalized one-off.
...
The model-facing DPO semantics remain the finalized plain-text one-off contract.
Only final accepted rows are adapted to the repository's public message-list schema.
"""
    ...
    decision = {
        ...
        "properties": {
            ...
            "accepted": {
                "type": "boolean",
                "description": (
                    "True only when chosen is genuinely preferable to rejected and "
                    "the preference difference is material and useful."
                ),
            },
            "reason": {
                "type": "string",
...
def generate_derivations(
    *,
    ...
    def request_derivations(request_count: int, prior: Sequence[str]) -> list[str]:
        ...
        prompt = f"""
You are generating semantic task classes for the DPO preference dimension:
{seed.dimension}.
...
- stay within the {seed.dimension} preference dimension;
- describe a setting where two independently plausible assistant responses could
  differ materially on {seed.dimension};
- favor realistic open-ended tasks, analysis, explanation, synthesis, correction,
  decision support, or constrained problem solving over quiz-like fact recall;
- avoid repeating common trivia themes (for example capitals, famous landmarks,
  simple arithmetic, elementary definitions) unless the derivation exercises a
  genuinely different reasoning or evidence-use failure mode;
...
derivations and reject it if the core preference distinction substantially overlaps.
        ...
        parsed, _ = call_structured_object(
            ...
            schema=exact_string_list_schema(
                ...
                description=(
                    ...
                    f"for the {seed.dimension} DPO preference dimension."
                ),
            ),
            schema_name="dpo_task_derivations",
        )
...
DPO preference dimension: {seed.dimension}
...
- Prefer tasks where a plausible assistant could make a subtle but material
  factual mistake: wrong inference, stale fact, misread evidence, unsupported
  certainty, omitted qualification, unit/definition confusion, or a locally
  incorrect step.
...
- Include all code, schemas, facts, labels, source text, or other material
  required to assess candidate responses when the task is intended to be
  self-contained.
...
""".strip()
        ...
        parsed, _ = call_structured_object(
            ...
            schema_name="dpo_concrete_tasks",
        )
...
Generate one CHOSEN and one REJECTED response for every task below.
...
- REJECTED must be locally plausible: most of it should be coherent and relevant,
  with one or a small number of material defects specifically affecting
  {dimension}.
- Prefer subtle, realistic defects such as a wrong inference, unsupported
  certainty, stale or slightly misapplied fact, incorrect qualification, mistaken
  unit/definition, incomplete evidence handling, or one consequential reasoning
  step.
- Do NOT create REJECTED by simply negating the correct answer, swapping an
  obviously true fact for an obviously false one, reversing basic definitions,
  inserting absurd arithmetic, or making every statement wrong.
- Do NOT make REJECTED random nonsense, empty, obviously absurd, unrelated, or
  intentionally self-contradictory.
- Do NOT make the distinction merely cosmetic, verbosity-only, formatting-only,
  or "detailed versus short".
- CHOSEN and REJECTED should usually share substantial correct content so the
  preference teaches discrimination rather than trivia.
- Both responses must independently answer the same user task.
- Do not rewrite the task.
- Do not mention that either response is intentionally good/bad, chosen/rejected,
  synthetic, or generated for preference training.
...
""".strip()
        ...
        parsed, _ = call_structured_object(
            ...
            schema_name="dpo_response_pairs",
        )
...
def judge_batch(
    *,
    ...
    prompt = f"""
Judge each DPO candidate pair below.

Evaluate the CHOSEN and REJECTED responses against the user's actual task and
the stated preference dimension.

Accept only when:
- the pair is assessable;
- CHOSEN is genuinely preferable to REJECTED;
- REJECTED is still plausible enough to be useful preference data;
...
Your job is only to decide whether each DPO acceptance is reasonably justified.
...
""".strip()
    ...
    parsed, _ = call_structured_object(
        ...
        schema_name="dpo_reviewer_batch",
    )
...
def run_dimension(args: argparse.Namespace) -> int:
    ...
    for row in final_rows:
        ...
        append_jsonl(out / "dpo.accepted.jsonl", row)
    ...
    summary = {
        **plan,
        ...
        "outputs": {
            ...
            "final_dataset": str(out / "dpo.accepted.jsonl"),
        },
    }
...
PUBLIC_DIMENSION = {
    "helpfulness_and_completeness": "helpfulness_and_completeness",
    "factual_accuracy": "factual_accuracy",
    "instruction_adherence": "instruction_adherence",
    "detail": "appropriate_detail",
    "organization": "organization",
    "style_and_tone": "style_and_tone",
    "tool_call_correctness": "tool_call_correctness",
    "groundedness": "groundedness",
    "safe_refusal": "safe_refusal_calibration",
...
TASK_FAMILY = {
    "code_correctness": "programming",
    "safe_refusal": "safety_uncertainty_and_refusal",
    "groundedness": "grounded_qa_and_reading",
    "factual_accuracy": "grounded_qa_and_reading",
}


def _public_metadata(dimension: str) -> dict[str, Any]:
    public_dimension = PUBLIC_DIMENSION[dimension]
    return {
        "task_family": TASK_FAMILY.get(dimension, "everyday_conversation"),
        "interaction_modes": ["single_turn"],
        "output_mode": "free_text",
        "context_mode": "self_contained",
        "difficulty": 3,
        "template_family": f"dpo_{public_dimension}",
        "preference_dimension": public_dimension,
        "failure_mode": FAILURE_MODE[dimension],
    }
...
def _resolve_dimensions(values: Sequence[str]) -> tuple[str, ...]:
    ...
    if not requested or requested == ("all",):
        return tuple(DPO_DIMENSIONS)
    unknown = sorted(set(requested) - set(DPO_DIMENSIONS))
    if unknown:
        raise ValueError(f"unknown DPO preference dimensions: {unknown}")
...
def _quality_evidence(work_dir: Path, accepted_ids: set[str]) -> dict[str, dict[str, Any]]:
    ...
    for public_id in sorted(accepted_ids):
        ...
        if not (
            evidence[public_id]["assessable"]
            and evidence[public_id]["judge_accepted"]
            and evidence[public_id]["reviewed"]
            and evidence[public_id]["reviewer_agreed"]
            and evidence[public_id]["accepted"]
        ):
            raise RuntimeError(f"accepted DPO row has non-passing quality evidence: {public_id}")
    return evidence


def _write_dimension_evidence_manifest(
    *,
    path: Path,
    dimension: str,
    ...
    payload = {
        ...
        "preference_dimension": public_dimension,
        "source_semantic_dimension": dimension,
        "dataset_path": str(dataset_path),
        "row_count": len(rows),
        "metadata": {
            "pipeline": "derivation_task_novelty_pair_validate_judge_reviewer_dedup",
            "deterministic_output_validation": {
                row_id: {"status": "passed"} for row_id in sorted(accepted_ids)
            },
...
def run_production(args: argparse.Namespace) -> int:
    ...
    attempted_per_dimension: dict[str, int] = {}
    accepted_per_dimension: dict[str, int] = {}
    rejected_per_dimension: dict[str, int] = {}
    duplicate_per_dimension: dict[str, int] = {}
    public_dimensions: list[str] = []
    ...
    for dimension in dimensions:
        ...
        print(f"[dpo-pipeline] dimension={dimension} start", flush=True)
        ...
        internal_rows = _read_jsonl(work_dir / "dpo.accepted.jsonl")
        public_rows = [_to_public_row(row, dimension) for row in internal_rows]
        if not public_rows:
            raise RuntimeError(f"DPO dimension produced no accepted rows: {dimension}")

        dataset_path = dataset_dir / f"{public_dimension}.jsonl"
        reset_output_files(dataset_dir, (dataset_path.name,))
        for row in public_rows:
            append_jsonl(dataset_path, row)

        evidence_manifest = _write_dimension_evidence_manifest(
            path=manifest_dir / f"{public_dimension}.{args.generation_run}.manifest.json",
            dimension=dimension,
        ...
        dimension_summaries[public_dimension] = dict(summary)
        attempted = int(summary.get("pairs_generated", 0) or 0)
        final_dedup = summary.get("final_dedup")
        duplicates = int(final_dedup.get("duplicate_rows", 0) or 0) if isinstance(final_dedup, Mapping) else 0
        attempted_per_dimension[public_dimension] = attempted
        accepted_per_dimension[public_dimension] = len(public_rows)
        duplicate_per_dimension[public_dimension] = duplicates
        rejected_per_dimension[public_dimension] = max(attempted - len(public_rows) - duplicates, 0)
        print(
            f"[dpo-pipeline] dimension={dimension} complete accepted={len(public_rows)} "
            f"dataset={dataset_path}",
            flush=True,
        )

    if args.dry_run:
        print("[dpo-pipeline] dry-run complete", flush=True)
        return 0

    total_attempted = sum(attempted_per_dimension.values())
    total_accepted = sum(accepted_per_dimension.values())
    total_duplicates = sum(duplicate_per_dimension.values())
    total_rejected = sum(rejected_per_dimension.values())
    estimated_tokens = sum(
        estimate_dpo_tokens(row)
        for dataset_path in sorted(dataset_dir.glob("*.jsonl"))
    ...
    run_manifest = {
        ...
        "metadata": {
            ...
            "candidate_pairs": total_attempted,
            "attempted_pairs": total_attempted,
            "accepted_pairs": total_accepted,
            "rejected_pairs": total_rejected,
            "duplicate_pairs": total_duplicates,
            "estimated_tokens": estimated_tokens,
            "candidate_pairs_per_dimension": attempted_per_dimension,
            "attempted_pairs_per_dimension": attempted_per_dimension,
            "accepted_pairs_per_dimension": accepted_per_dimension,
            "rejected_pairs_per_dimension": rejected_per_dimension,
            "duplicate_pairs_per_dimension": duplicate_per_dimension,
    ...
    print(
        f"[dpo-pipeline] complete dimensions={len(public_dimensions)} accepted={total_accepted} "
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Production generic DPO pipeline migrated from the finalized one-off."
    )
